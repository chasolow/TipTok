...
# Поля для ввода мощности с использованием слайдера
if power_question == '⚡️ Да':
    P = st.slider("Введите суммарную мощность объекта (P):", min_value=0, max_value=1000, step=10, value=0)
    Pdop = st.slider("Введите дополнительную мощность (Pдоп):", min_value=0, max_value=1000, step=10, value=0)
else:
    P = st.slider("Введите суммарную мощность объекта (P):", min_value=0, max_value=1000, step=10, value=0)
    Pdop = None
...
if schemes == '✅ Есть':
    X = st.slider("Количество участков ЛЭП от центра питания до каждого электроприемника:", min_value=0, max_value=100, step=1, value=0)
    Y = st.slider("Количество электроприемников:", min_value=0, max_value=100, step=1, value=0)
else:
    Y = st.slider("Количество электроприемников:", min_value=0, max_value=100, step=1, value=0)
    X = Y * 1.05
